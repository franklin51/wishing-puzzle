# Project Skeleton (index.html, style.css, main.js)

> Minimal runnable starter with HTML5 Canvas, responsive layout, basic state model, drag logic for top-of-stack card, candle progress rendering, and TODO hooks for wishes/mic/camera/export. Drop these three files into a folder and open `index.html`.
...
.topbar, .bottombar { display:flex; align-items:center; justify-content:space-between; padding: 8px 16px; }
.topbar { background: var(--paper); border-bottom: 1px solid #e8e2da; }
.bottombar { background: transparent; gap: 8px; }
.stage { display:grid; place-items:center; padding: 8px 0; }
canvas#scene { width: min(96vw, 1280px); height: calc(min(96vw, 1280px) * 9 / 16); background: var(--beige); border-radius: 16px; box-shadow: 0 20px 40px rgba(0,0,0,.06);
}
.btn { padding: 8px 14px; border-radius: 10px; border: 1px solid #d7d0c6; background: #fff; color: var(--ink); cursor: pointer; }
.btn.primary { background: var(--accent); color: white; border-color: var(--accent); }
.btn.subtle { background: transparent; }
.btn:disabled { opacity:.5; cursor: not-allowed; }
.progress { color: var(--muted); font-weight: 600; }

.overlay { position: fixed; inset: 0; display:grid; place-items:center; background: rgba(0,0,0,.35); padding: 24px; }
.overlay.hidden { display: none; }
.panel { width:min(92vw, 720px); background: var(--paper); border-radius: 16px; padding: 16px; box-shadow: 0 16px 40px rgba(0,0,0,.2); }
.panel h2 { margin-top:0; }
.panel label { display:block; margin: 8px 0; }
.panel input { width:100%; padding:10px 12px; border:1px solid #e1d9cf; border-radius: 10px; background: #fff; }
...
### Notes / Next Steps
- Replace `cards[i].text` with your 27-line copy.
- Implement microphone-based blow detection (Web Audio API):
  - Analyze time-domain volume; threshold triggers flame extinguish animation → then `openCamera()`.
- Add wish text rendering onto the left panel (as stickers near the cake area).
- Implement high-res export composition into `exportCanvas` (2480×3508) before saving JPG/PDF.
- Optional: add background music + SFX with Web Audio.
